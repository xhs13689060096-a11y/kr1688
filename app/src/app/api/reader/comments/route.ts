...
export async function POST(request: Request) {
  try {
    ...
    const comment = await context.payload.create({ collection: 'comments', draft: false, req: context.req, overrideAccess: false, data: { author: context.user.id, story: chapter.story, chapter: chapter.id, body: { root: { type: 'root', children: [{ type: 'paragraph', children: [{ type: 'text', text: body.trim() }] }] } } } })
