...
### Task 3: Create and verify the initial production migration

**Files:**
- Create: one `app/src/migrations/` timestamped `initial_schema.ts` file generated by Payload
- Create: `app/src/migrations/index.ts`
- Create: `app/tests/int/production-migration.test.ts`
- Modify: `app/package.json`
- Modify: `app/pnpm-lock.yaml`
- Modify: `app/docs/executor/STATUS.yaml`

**Consumes:** the accepted Payload configuration and GitHub Actions' disposable
PostgreSQL service. Migration generation must not connect to Neon production.

**Produces:** a committed initial schema migration and a `vercel-build` command
that fails the deployment before build if migration fails.

- [ ] **Step 1: Write the failing migration contract test**

Using a real `pg` client and `DATABASE_URL`, query PostgreSQL after the migration
command and assert that the required Payload tables for users, stories,
chapters, comments, media, favorites, and reading progress exist. The test must
not initialize Payload because non-production initialization could push schema
and create a false positive.

- [ ] **Step 2: Run the focused test and confirm RED**

Run: `pnpm exec vitest run tests/int/production-migration.test.ts --config vitest.config.mts`

Expected locally: the known PostgreSQL-unavailable diagnostic. Expected on the
first GitHub Actions RED SHA: FAIL because all required tables remain absent
after `payload migrate` finds no committed migration.

- [ ] **Step 3: Generate the initial migration without a database connection**

Run: `pnpm payload migrate:create initial_schema --skip-empty`

Environment: use only `app/test.env`. Payload's `migrate:create` initializes
with database connection disabled; it must not receive the Neon connection.

Expected: Payload creates the timestamped TypeScript migration and
`src/migrations/index.ts`.

- [ ] **Step 4: Add the production build boundary**

Add `"vercel-build": "pnpm payload migrate && pnpm build"` to `app/package.json`.
Do not change the ordinary `build` command used by local and CI build checks.
Add `pg` and its TypeScript declarations as direct development dependencies for
the migration boundary test.

- [ ] **Step 5: Verify GREEN against disposable PostgreSQL**

Update `verify:ci` so it first executes `pnpm payload migrate`, then runs the
focused production migration test before any integration test can initialize
Payload. Commit and push the test-only RED state before generating the migration.
After the exact RED run proves the required tables are absent, generate the
migration, add `vercel-build`, commit, push, and require the exact GitHub Actions
SHA to succeed against its PostgreSQL service.

Expected: migration applies to an empty disposable database; all integration,
Chrome E2E, build, lint, and guardrail checks remain green.

- [ ] **Step 6: Record the migration gate**

Update `STATUS.yaml` with the migration SHA and exact successful CI URL without
including any credential or connection detail.

---

### Task 4: Deploy and verify the new production candidate
...
### Task 5: Cut over the canonical domain and retire the old project
...
### Task 6: Final independent launch review
